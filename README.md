...
- **Company Similarity Engine (v9.2)** — Python-in-Excel tool using TF-IDF (bigrams) + LSA to find comparable public companies from business descriptions. 5-cell architecture: Utilities → Engine → Orchestrator → Loader → Execution, using a dict registry pattern injected via `builtins._SIM_ENGINE`. Features 4-level GICS hierarchy boosting, region filtering, keyword boosting, and configurable matching strategies. Scores on 0–100 scale. See `SimilarityEngine_PythonGuide_v1.docx` for foundational documentation (note: guide covers v8 architecture; v9.2 adds the Loader cell and dict registry pattern).
